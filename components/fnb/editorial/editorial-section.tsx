...
export function EditorialSection({ id, index, title, children, aside, className }: EditorialSectionProps) {
  const headingId = `${id}-heading`
  return (
    <section id={id} aria-labelledby={headingId} className={`border-t border-steel/40 ${className ?? 'bg-obsidian'}`}>
      <div className="mx-auto grid max-w-[1600px] gap-8 px-5 py-16 md:px-10 md:py-24 lg:grid-cols-[minmax(12rem,0.38fr)_minmax(0,1fr)] lg:gap-16">
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(12rem,0.42fr)]">
          ...
          {aside ? <aside className="border-l border-signal/50 pl-5 text-sm leading-relaxed text-ash">{aside}</aside> : null}
        </div>
      </div>
    </section>
  )
