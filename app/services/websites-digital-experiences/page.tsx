...
export default function DigitalExperiencesPage() {
  ...
  return (
    <main id="main">
      <EditorialHero eyebrow={content.hero.eyebrow} title={content.hero.title} lead={content.hero.lead}><Link href={content.hero.anchorHref!} className="fnb-btn-ghost">{content.hero.anchorLabel} <span aria-hidden="true">&#8595;</span></Link></EditorialHero>
      ...
      <EditorialSection {...s.ui}><ServiceCopy paragraphs={s.ui.body} /></EditorialSection>
      <ServiceMediaFeature {...s.web} asset={getTask4ServiceMedia(content)} mediaPosition={content.media.mediaPosition} portrait={content.media.mediaAspect === 'portrait'} boundary={content.media.boundary} disclosure={content.media.disclosure}><ServiceCopy paragraphs={s.web.body} /></ServiceMediaFeature>
      <EditorialSection {...s.storytelling} className="bg-obsidian"><ServiceCopy paragraphs={s.storytelling.body} /></EditorialSection>
      <section id={s.responsive.id} aria-labelledby={`${s.responsive.id}-heading`} className="border-t border-steel/40 bg-void"><div className="mx-auto max-w-[1600px] px-5 py-16 md:px-10 md:py-24"><p className="fnb-label text-signal">{s.responsive.index}</p><h2 id={`${s.responsive.id}-heading`} className="fnb-head mt-5 max-w-[16ch] text-3xl text-warm-white md:text-5xl">{s.responsive.title}</h2><div className="mt-10"><ServiceScopeList ariaLabel={s.responsive.ariaLabel!} items={s.responsive.items!} /></div></div></section>
      <EditorialSection {...s.conversion}><ServiceCopy paragraphs={s.conversion.body} /></EditorialSection>
