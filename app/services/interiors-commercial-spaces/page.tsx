...
export default function InteriorsCommercialSpacesPage() {
  ...
  return (
    <main id="main">
      ...
      <section id={s.strategy.id} aria-labelledby={`${s.strategy.id}-heading`} data-service-block={getTask4ServiceBlock(content, 1)} data-service-kind="flow" className="border-t border-steel/40 bg-obsidian"><div className="mx-auto grid max-w-[1600px] gap-10 px-5 py-16 md:px-10 md:py-24 lg:grid-cols-[minmax(0,0.6fr)_minmax(0,1fr)] lg:gap-16"><div><p className="fnb-label text-signal">{s.strategy.index}</p><h2 id={`${s.strategy.id}-heading`} className="fnb-head mt-5 max-w-[13ch] text-3xl text-warm-white md:text-5xl">{s.strategy.title}</h2></div><ServiceDecisionFlow label={s.strategy.ariaLabel!} steps={s.strategy.flow!} /></div></section>
      <EditorialSection {...s.planning} serviceBlock={getTask4ServiceBlock(content, 2)} className="bg-void"><ServiceCopy paragraphs={s.planning.body} /></EditorialSection>
      <ServiceMediaFeature {...s.materials} serviceBlock={getTask4ServiceBlock(content, 3)} asset={getTask4ServiceMedia(content)} mediaPosition={content.media.mediaPosition} portrait={content.media.mediaAspect === 'portrait'} rhythm={content.media.rhythm} boundary={content.media.boundary} disclosure={content.media.disclosure}><ServiceCopy paragraphs={s.materials.body} /></ServiceMediaFeature>
      <EditorialSection {...s.brand} serviceBlock={getTask4ServiceBlock(content, 4)}><ServiceCopy paragraphs={s.brand.body} /></EditorialSection>
      <section id={s.commercial.id} aria-labelledby={`${s.commercial.id}-heading`} data-service-block={getTask4ServiceBlock(content, 5)} data-service-kind="scope-list" className="border-t border-steel/40 bg-void"><div className="mx-auto max-w-[1600px] px-5 py-16 md:px-10 md:py-24"><p className="fnb-label text-signal">{s.commercial.index}</p><h2 id={`${s.commercial.id}-heading`} className="fnb-head mt-5 max-w-[16ch] text-3xl text-warm-white md:text-5xl">{s.commercial.title}</h2><div className="mt-10"><ServiceScopeList ariaLabel={s.commercial.ariaLabel!} items={s.commercial.items!} /></div></div></section>
      <EditorialSection {...s.development} serviceBlock={getTask4ServiceBlock(content, 6)}><ServiceCopy paragraphs={s.development.body} /></EditorialSection>
